...
def subscribe(listname):
    ...
    **Parameters**:
    ...
    try:
        mlist.ApprovedAddMember(userdesc, admin_notif=True)
...
def unsubscribe(listname):
    ...
    **Parameters**:
    ...
    try:
        mlist.ApprovedDeleteMember(address, admin_notif=False)
        mlist.Save()
...
def members(listname):
    ...
    **URI**: /<listname>
    ...
    mlist = get_mailinglist(listname, lock=False)
    include_name = request.query.get('name')
    member_list = mlist.getMembers()
    if include_name:
        result_list = [ (mlist.getMemberCPAddress(member), mlist.getMemberName(member)) for member in member_list]
